jQuery(function () {
    ...
    var $ = jQuery, // just in case. Make sure it's not an other libaray.
            btnUploadSelector = "#btnSubmit",
            domForm = $("#patient-form"), // form dom
            btnSubmit = $("#btnSubmit"), // btnSubmit dom.    
            $wrap = $('#uploader'),
            // 图片容器
            ...
            // 上传按钮
            $upload = $wrap.find(btnUploadSelector),
            // 没选择文件之前的内容。
    ...
    uploader = WebUploader.create({
        pick: {
            id: '#filePicker',
            label: '<i class="fa fa-camera"></i>&nbsp;选择病历'
        },
...
//    });
    ...
    uploader.addButton({
        id: '#filePicker2',
        label: '<i class="fa fa-camera"></i>&nbsp;继续添加'
    });
...
//                break;
            ...
            case 'finish':
                stats = uploader.getStats();
                if (stats.successNum) {
                    uploadReturnUrl += "?pid=" + fileParam.id;
...
//                break;
    ...
    uploader.onError = function (code) {
        ...
        $("#tipPage .tipcontent p").text(errorinfo);
        $("#toTip").trigger("click");
        //window.location.hash = '#tipPage';
...
//表单验证板块
    var validator = domForm.validate({
        ...
        messages: {
            ...
            'patient[remark]': {
                maxlength: '请将字数控制在500以内'
            }
        },
        //    errorContainer: "div.error",
        //    errorLabelContainer: $("#patient-form div .error"),
        //wrapper: "div",
        errorElement: "div",
        ...
        submitHandler: function () {
            $(".ui-loader").show();
            //form插件的异步无刷新提交
            actionUrl = domForm.attr('actionl');
            ...
            domForm.ajaxSubmit({
                ...
                success: function (data) {
                    ...
                    if (data.status == 'ok') {
                        //基本数据插入成功  判断是否有图片
                        if (state == 'ready') {
                            fileParam.id = data.patient.id;
                            //文件上传 自动跳转
                            uploader.upload();
                        } else {
                            //没有上传文件 表单数据添加成功 页面跳转
                            window.location.href = uploadReturnUrl + "?pid=" + data.patient.id;
