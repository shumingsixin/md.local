    ...
    uploader.on("startUpload", function () {
        ...
        if (fileParam.id != '' && fileParam.id != null) {
            uploader.option("formData", {
                'doctor[id]': fileParam.id,
                //'doctor[name]': fileParam.name
            });
        }
...
//        errorLabelContainer: $("#doctor-form div .error"),
//        wrapper: "div",
        errorElement: "div",
        errorPlacement: function (error, element) {                             //错误信息位置设置方法  
            if (error[0].id == 'doctor_state_id-error' || error[0].id == 'doctor_city_id-error' || error[0].id == 'doctor_medical_title-error' || error[0].id == 'doctor_academic_title-error') {
