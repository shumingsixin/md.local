 ...
 * @since 1.0
 */
class EUploadedFile extends CComponent {
     ...
     */
    protected static function collectFilesRecursive($key, $names, $tmp_names, $types, $sizes, $errors, $index=0) {
        if (is_array($names)) {
            foreach ($names as $item => $name) {
                self::collectFilesRecursive($key . '[' . $item . ']', $names[$item], $tmp_names[$item], $types[$item], $sizes[$item], $errors[$item], $item);
            }
        }
        else
            self::$_files[$key] = new EUploadedFile($names, $tmp_names, $types, $sizes, $errors, $index);
    }
     ...
     */
    public function __construct($name, $tempName, $type, $size, $error, $index) {
        $this->_name = $name;
